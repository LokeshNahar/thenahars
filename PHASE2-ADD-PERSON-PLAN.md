...
**Verified constraint (found while testing against real deployed rules on `thenahars-dev`, not just designed on paper): a Firestore security rule's `get()`/`exists()` call cannot see another write from the SAME atomic commit** (transaction or batch) — so "create the new person AND splice them into the anchor's array" cannot be one atomic operation, since the reverse-link update's rule needs to `get()` the newly-created person to verify `addedBy`. Confirmed by direct testing: the identical create+update pair fails together via both `runTransaction` and `writeBatch`, but each succeeds independently, and the update succeeds immediately once the created document exists from a prior, separate commit. **Fix: two sequential commits, not one.** Commit 1 = `runTransaction` reserving the id + creating the person (self-contained, no cross-doc dependency). Commit 2 = a plain `updateDoc` splicing the new id into the anchor's array, executed immediately after commit 1 resolves. There's a small window (milliseconds, same client call) where the person exists but isn't yet linked; acceptable, and detectable/retryable if commit 2 ever fails.
